...
def get_text_from_byparms_file():
    ...
    byparms_file = custom_tools.getfile('select the byonic parameter file', '.byparms')

    text_out = 'byparms file = "{}"\n\n'.format(byparms_file) + parse_byparms_df(byparms_file)

    save_path = os.path.join(os.path.dirname(byparms_file), 'byparms_as_text_{}.txt'.format(get_time()))
...
def byparms_into_df(byparms_filepath, test=False):
    if test:
        open_file = r'test.byparms'
    else:
        open_file = byparms_filepath # custom_tools.getfile('select the byonic parameter file', '.byparms')

    return pd.read_table(open_file, skiprows=range(9), delimiter='=')


def parse_byparms_df(byparms_filepath):
    # initialise the desired dictionary keys
    salient_dict_keys = initialise_salient_dict_keys()

    # get the byparms as df
    df_byparms = byparms_into_df(byparms_filepath, test=False)
...
def write_byparms_as_method_text(salient_dict):
    ...
    results[
        ...
        **salient_dict)
    results[
        'advanced'] = 'Precursor off-by-x was set to {off_by_one}. FDR threshold was {protein_fdr_pulldown}.'.format(
        **salient_dict)
